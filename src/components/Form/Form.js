  ...
  return (
    <Paper className="paper">
      <form autoComplete="off" noValidate className="root" form onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post.title}"` : 'Credentials'}</Typography>
        ...
        <TextField name="id" variant="outlined" label="Id" fullWidth value={postData.title} onChange={(e) => setPostData({ ...postData, title: e.target.value })} />
        <TextField name="Day" variant="outlined" label="Day" fullWidth multiline rows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} />
